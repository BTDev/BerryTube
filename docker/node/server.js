...
process.stdin.on('data', function (chunk) {
	try {
		...
		DefaultLog.error(events.EVENT_REPL, "error invoking repl script: {script}", {
			script: chunk
		}, e);
	}
});
...
DefaultLog.addLogger(
	...
	createStreamLogger({
		[levels.LEVEL_DEBUG]: fs.createWriteStream(SERVER.settings.core.debug_file_name, {
			flags: "w"
		}),
		[levels.LEVEL_INFORMATION]: fs.createWriteStream(SERVER.settings.core.log_file_name, {
			flags: "w"
		}),
		[levels.LEVEL_ERROR]: fs.createWriteStream(SERVER.settings.core.error_file_name, {
			flags: "a"
		})
	}),
	...
	async (logEvent) => {
		...
		const adminMessage = {
			...
			type: data.type || "site",
			nick: data.mod,
			berry: sessionService.berrySession && sessionService.berrySession.nick === data.mod,
...
function initPlaylist(callback) {
	...
	mysql.query(sql, function (err, result) {
		if (err) {
			DefaultLog.error(events.EVENT_DB_QUERY, "query \"{sql}\" failed", {
				sql
			}, err);
			return;
		}
...
function initResumePosition(callback) {
	getMisc({
		name: 'server_active_videoid'
	}, function (old_videoid) {
...
function augmentBan(ban, o) {
	...
	if (o.bannedOn < ban.bannedOn) {
		ban.bannedOn = o.bannedOn;
	}

	// Take all special values direct, otherwise, replace only if longer period.
	if (o.duration <= 0) {
		ban.duration = o.duration;
	} else if (o.duration > ban.duration) {
		ban.duration = o.duration;
	}
}
...
function isUserBanned(o) {
	...
	for (bannedguy in SERVER.BANS) {
		...
		// Check all IP's
		for (ip in o.ips) {
			if (!SERVER.BANS[bannedguy].ips) {
				SERVER.BANS[bannedguy].ips = [];
			}
		...
		// Check all Nicks
		for (nick in o.nicks) {
			if (!SERVER.BANS[bannedguy].nicks) {
				SERVER.BANS[bannedguy].nicks = [];
			}
...
var commit = function () {
	...
	upsertMisc({
		name: 'filters',
		value: JSON.stringify(SERVER.FILTERS)
	});
	...
	upsertMisc({
		name: 'shadowbant_ips',
		value: JSON.stringify(shadowbant)
	});
	upsertMisc({
		name: 'hardbant_ips',
		value: JSON.stringify(SERVER.BANS)
	});
	upsertMisc({
		name: 'server_time',
		value: '' + Math.ceil(SERVER.TIME)
	});
	upsertMisc({
		name: 'server_active_videoid',
		value: '' + SERVER.ACTIVE.videoid
	});
};
	...
	try {
		for (var i = 0; i < SERVER.FILTERS.length; i++) {
			...
			try {
				...
				DefaultLog.error(events.EVENT_ADMIN_APPLY_FILTERS, "could not apply filter {filterId} to chat message", {
					filterId: i
				}, e);
			...
			if (nick.match(nickCheck)) {
				if (msg.match(chatCheck)) {
					// Perform Action
					actionChain.push({
						action: d.actionSelector,
						meta: d.actionMetadata
					});
				}
...
const doNormalChatMessage = {
	doSuppress: false
};
const doSuppressChat = {
	doSuppress: true
};
...
function _sendChat(nick, type, incoming, socket) {
	...
	if (command) {
		const {
			doSuppress
		} = command(parsed, socket, messageData);
...
function getToggleable(name) {
	if (typeof SERVER.settings.toggles[name] == "undefined") {
		DefaultLog.error(events.EVENT_GENERAL, "No such toggleable {name} found", {
			name
		});
		return false;
	}
...
function rawAddVideo(d, successCallback, failureCallback) {
	...
	mysql.query(sql, ['' + d.videoid], function (err, result) {
		...
		if (!('meta' in d) || d.meta == null) {
			d.meta = {};
		}
		if (!('addedon' in d.meta)) {
			d.meta.addedon = new Date().getTime();
		}
...
function addVideoYT(socket, data, meta, successCallback, failureCallback) {
	...
	var req = https.request(options, function (res) {
		...
		res.on('end', function () { //7zLNB9z_AI4
			try {
				...
				maybeError = e;
				DefaultLog.error(events.EVENT_ADMIN_ADDED_VIDEO, "could not add youtube video {videoId}: {error}... trying fallback", {
					videoId: videoid,
					error: e.message || e
				}, err);
			...
			if (vidObj && vidObj.items && vidObj.items.length > 0) {
				...
				DefaultLog.error(events.EVENT_ADMIN_ADDED_VIDEO, "could not add youtube video {videoId}: {response}... trying fallback", {
					videoId: videoid,
					response: JSON.stringify(vidObj)
				});
				addYoutubeVideoFallback();
				return;
			}
			...
			for (var hasProperties in restrictReasons) {
				break;
			}
			if (hasProperties) {
	...
	function addYoutubeVideoFallback() {
		fetchYoutubeVideoInfo(videoid, (err, videoData) => {
			if (err) {
				DefaultLog.error(events.EVENT_ADMIN_ADDED_VIDEO, "could not add youtube video {videoId}: {error}", {
					videoId: videoid,
					error: err
				}, err);
				failureCallback(err);
	...
	followRedirect(options, function (res) {
		...
		res.on('end', function () {
			...
			rawAddVideo({
				...
				if (successCallback) {
					successCallback({
						title: jdata.user.username + " - " + jdata.title
					});
				}
			}, function (err) {
				if (failureCallback) {
...
function addVideoTwitch(socket, data, meta, successCallback, failureCallback) {
	...
	if (parts[0] === 'videos') {
		...
		twitchApi(['search', 'channels'], {
			...
			if (!response) {
				if (failureCallback) {
					failureCallback('no such channel');
				}
				return;
			}
...
initFilters();
initAreas();
DefaultLog.info(events.EVENT_SERVER_STATUS, "server version {version} started up", {
	version: SERVER.settings.core.version
});
...
io.sockets.on('connection', function (ioSocket) {
			...
			if (result.length >= 1) {
				...
				bcrypt.hash(data.pass, SERVER.settings.core.bcrypt_rounds, function (err, hash) {
					if (err) {
						...
						DefaultLog.error(events.EVENT_GENERAL, "Failed to bcrypt for {nick}'s password", {
							nick: data.nick
						}, err);
						return;
					}
	...
	socket.on("changePassword", function (data) {
		...
		if (!nick) {
			DefaultLog.error(events.EVENT_GENERAL, "Failed to get nick from socket on ip {ip}", {
				ip: socket.ip
			});
			return;
		}
	...
	socket.on("addVideo", function (data) {
		...
		const meta = {
			nick: socket.session.nick,
			type: socket.session.type
		};
		const logData = {
			mod: getSocketName(socket),
			type: "playlist",
			title: data.videotitle || data.videoid,
			provider: data.videotype
		};
	...
	socket.on("shadowBan", function (data) {
		...
		if (isbanning) {
			var banEmotes = ['[](/ihavenomouthandimustscream)', '[](/bant)', '[](/mmmbananas)', '[](/celbanned)', '[](/seriouslybanned)', '[](/konahappy)', '[](/ppshutup)', '[](/bpstfu)', '[](/eatadick)', '[](/suggestionbox)', '[](/rargtfo)', '[](/fuckyoudobby)', '[](/cleese)', '[](/wingflipoff)', '[](/pokemonkilledmyparents)', '[](/fuckyourshit)'];
			message = banEmotes[Math.floor(Math.random() * banEmotes.length)] + ' ' + message;
		}
		message = '/me ' + message;
		_sendChat(socket.session.nick, 3, {
			msg: message,
			metadata: {
				channel: 'admin'
			}
					...
					mysql.query(sql, [d.nick], function (err, result, fields) {
						...
						if (result.length == 1) {
							...
							try {
								...
								DefaultLog.error(events.EVENT_GENERAL, "Failed to parse user meta for {nick}", {
									nick: d.nick
								}, e);
								meta = {};
							}
