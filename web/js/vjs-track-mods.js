...
function resyncAudio(){
	setTimeout(()=>{this.currentTime(this.currentTime()-0.3);},1500);
	setTimeout(()=>{this.currentTime(this.currentTime()+0.5);},1700);
}
	...
	vjs.ready(function(){
		this.one("loadedmetadata", ()=>{
			...
			if (doAudioTracks) {
				...
				this.audioTracks().on('change', resyncAudio.bind(this));
			}
