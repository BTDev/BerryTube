...
function parseVideoURL(url,callback){
	...
	var m = url.match(new RegExp("v\.redd\.it\/([^\/]+)")); if (m) { callback(`video:${m[1]}`, "reddit"); return; }
	var m = url.match(new RegExp("^https:\/\/(www\.)?reddit\.com")); if (m) { callback(`post:${url}`, "reddit"); return; }
